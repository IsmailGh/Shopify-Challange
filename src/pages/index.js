...
import { Button, SourceCode } from "../components";
...
const HomePage = () => {
    ...
    const fetchData = async () => {
      ...
      if (data.n) {
        ...
        const responses = await Promise.all(
          array.map((n) =>
            axios(
              `https://www.omdbapi.com/?i=${n}&type=movie&apikey=${process.env.REACT_APP_OMDB_API_KEY_2}`
            )
          )
        );
  ...
  const isNominated = (id) =>
    nominations.filter((n) => n.imdbID === id).length > 0;
  ...
  const updateUrl = (noms) => {
    ...
    if (data.s) {
      window.history.pushState(
        "",
        "",
        `${window.location.origin}/?s=${data.s}&${str}`
      );
    } else {
